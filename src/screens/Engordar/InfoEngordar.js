...
export default function App(props){
 ...
 useEffect(()=>{
  ...
  db.collection('Engordar').get().then((snap)=>{
 ...
 const listData = [];
 db.collection('Engordar').orderBy('startAt', 'desc').limit(limitData).get().then((response)=>{
  ...
  db.collection('Engordar').orderBy('startAt', 'desc')
  ...
  const alertNoForm = () =>{
    ...
    return(
      <View style={{backgroundColor:'#CCD6F2', zIndex:-1}}>
...
function FormData(props){
  ...
  const Delete = ()=>{
    ...
    Alert.alert(
     ...
     const remove =()=>{
      ...
      db.collection('Engordar').doc(id).delete().then(function() {
          //console.log("Document successfully deleted!");
          navigation.navigate('engordar');
  ...
  return(
     ...
     <ScrollView style={styles.body}>
            ...
            <LinearGradient  colors={['#CCF2FB', '#DAEDF1']} key={id} style={styles.container}> 
            
                
                <View style={styles.header}>
                    <Text style={styles.title}>Engordar </Text>
           ...
           <View style={styles.bodyContainer}>
                  ...
                  <View style={styles.ViewContainer}>
                    <Text style={styles.TextContainer}>No. Lechones: </Text>
                    <Text style={styles.AnswerContainer}>{NoLechones}</Text>
                  </View>

                  <View style={styles.ViewContainer}>
                    <Text style={styles.TextContainer}>Fecha Destete: </Text>
                    <Text style={styles.AnswerContainer}>{Fecha}</Text>
                  </View>

                  <View style={styles.ViewContainer}>
                    <Text style={styles.TextContainer}>Inicio: </Text>
                    <Text style={styles.AnswerContainer}>{Inicio}</Text>
                  </View>

                  <View style={styles.ViewContainer}>
                    <Text style={styles.TextContainer}>Crecimiento: </Text>
                    <Text style={styles.AnswerContainer}>{Crecimiento}</Text>
                  </View>

                  <View style={styles.ViewContainer}>
                    <Text style={styles.TextContainer}>Desarrollo: </Text>
                    <Text style={styles.AnswerContainer}>{Desarrollo}</Text>
                  </View>

                  <View style={styles.ViewContainer}>
                    <Text style={styles.TextContainer}>Engorda: </Text>
                    <Text style={styles.AnswerContainer}>{Engorda}</Text>
                  </View>
